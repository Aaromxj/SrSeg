...
from core.criterion import CrossEntropy, OhemCrossEntropy, SSRCrossEntropy, OhemSSRCrossEntropy
...
def main():
    ...
    if config.LOSS.USE_OHEM:
        ...
        criterion = OhemSSRCrossEntropy(ignore_label=config.TRAIN.IGNORE_LABEL,
                                     thres=config.LOSS.OHEMTHRES,
                                     min_kept=config.LOSS.OHEMKEEP,
                                     weight=train_dataset.class_weights)
    else:
        ...
        #                         weight=train_dataset.class_weights)
        criterion = SSRCrossEntropy(ignore_label=config.TRAIN.IGNORE_LABEL,
                                 weight=train_dataset.class_weights)
    ...
    for epoch in range(last_epoch, end_epoch):
        ...
        if args.local_rank == 0:
            ...
            msg = 'Loss: {:.3f}, MeanIU: {: 4.4f}, Best_mIoU: {: 4.4f}'.format(
                    valid_loss/config.TRAIN.BATCH_SIZE_PER_GPU, mean_IoU, best_mIoU)
            logging.info(msg)
