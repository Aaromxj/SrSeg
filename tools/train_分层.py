...
def main():
    ...
    srnet1 = list(map(id, model.sub_mean.parameters()))
    srnet2 = list(map(id, model.add_mean.parameters()))
    srnet3 = list(map(id, model.head.parameters()))
    srnet4 = list(map(id, model.body.parameters()))
    srnet5 = list(map(id, model.tail.parameters()))


    if config.TRAIN.OPTIMIZER == 'sgd':
        optimizer = torch.optim.SGD([{'params':
                                  filter(lambda p: p.requires_grad and id(p) not in srnet1 + srnet2 + srnet3 + srnet4 + srnet5,
                                         model.parameters()),
                                  'lr': config.TRAIN.LR},
                                {'params': model.sub_mean.parameters(), 'lr': 5e-5},
                                {'params': model.add_mean.parameters(), 'lr': 5e-5},
                                {'params': model.head.parameters(), 'lr': 5e-5},
                                {'params': model.body.parameters(), 'lr': 5e-5},
                                {'params': model.tail.parameters(), 'lr': 5e-5}],
                                lr=config.TRAIN.LR,
